 ...
 *
 */
public class ChunkGeneratorMorphCraft implements IChunkGenerator {
	...
	private MapGenWoodlandMansion woodlandMansionGenerator = new MapGenWoodlandMansion(this);
	...
	public ChunkGeneratorMorphCraft(World worldIn, long seed, boolean mapFeaturesEnabledIn, String generatorOptions) {
		{
			...
			woodlandMansionGenerator = (MapGenWoodlandMansion) net.minecraftforge.event.terraingen.TerrainGen.getModdedMapGen(woodlandMansionGenerator, net.minecraftforge.event.terraingen.InitMapGenEvent.EventType.WOODLAND_MANSION);
		}
		world = worldIn;
		...
		if (generatorOptions != null) {
			...
			oceanBlock = settings.useLavaOceans ? Blocks.LAVA.getDefaultState() : Blocks.AIR.getDefaultState();
	...
	public void populate(int x, int z) {
		...
		if (biome != Biomes.DESERT && biome != Biomes.DESERT_HILLS && settings.useWaterLakes && !flag && rand.nextInt(settings.waterLakeChance) == 0) {
			if (net.minecraftforge.event.terraingen.TerrainGen.populate(this, world, rand, x, z, flag, net.minecraftforge.event.terraingen.PopulateChunkEvent.Populate.EventType.LAKE)) {
		...
		if (!flag && rand.nextInt(settings.lavaLakeChance / 10) == 0 && settings.useLavaLakes) {
			if (net.minecraftforge.event.terraingen.TerrainGen.populate(this, world, rand, x, z, flag, net.minecraftforge.event.terraingen.PopulateChunkEvent.Populate.EventType.LAVA)) {
		...
		if (settings.useDungeons) {
			if (net.minecraftforge.event.terraingen.TerrainGen.populate(this, world, rand, x, z, flag, net.minecraftforge.event.terraingen.PopulateChunkEvent.Populate.EventType.DUNGEON)) {
		...
		if (net.minecraftforge.event.terraingen.TerrainGen.populate(this, world, rand, x, z, flag, net.minecraftforge.event.terraingen.PopulateChunkEvent.Populate.EventType.ANIMALS)) {
			WorldEntitySpawner.performWorldGenSpawning(world, biome, i + 8, j + 8, 16, 16, rand);
		}
		blockpos = blockpos.add(8, 0, 8);

		if (net.minecraftforge.event.terraingen.TerrainGen.populate(this, world, rand, x, z, flag, net.minecraftforge.event.terraingen.PopulateChunkEvent.Populate.EventType.ICE)) {
	...
	public boolean isInsideStructure(World worldIn, String structureName, BlockPos pos) {
		...
		else if ("Mansion".equals(structureName) && woodlandMansionGenerator != null) {
	...
	public BlockPos getNearestStructurePos(World worldIn, String structureName, BlockPos position, boolean findUnexplored) {
		if (!mapFeaturesEnabled) {
			...
			return "Temple".equals(structureName) && scatteredFeatureGenerator != null ? scatteredFeatureGenerator.getNearestStructurePos(worldIn, position, findUnexplored) : null;
		}
	}
	...
	public void recreateStructures(Chunk chunkIn, int x, int z) {
		if (mapFeaturesEnabled) {
			...
			if (settings.useMansions) {
				woodlandMansionGenerator.generate(world, x, z, (ChunkPrimer) null);
			}
		}
	}
