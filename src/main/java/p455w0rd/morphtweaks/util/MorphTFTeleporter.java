 ...
 *
 */
public class MorphTFTeleporter extends Teleporter {

	public static MorphTFTeleporter getTeleporterForDim(MinecraftServer server, int dim) {
		WorldServer ws = server.getWorld(dim);

		for (Teleporter t : ws.customTeleporters) {
			if (t instanceof MorphTFTeleporter) {
				return (MorphTFTeleporter) t;
			}
		}

		MorphTFTeleporter tp = new MorphTFTeleporter(ws);
		ws.customTeleporters.add(tp);
		return tp;
	}

	private MorphTFTeleporter(WorldServer dest) {
		super(dest);
	}
		...
		if (!placeInExistingPortal(par1Entity, facing)) {
			...
			if (par1Entity.world.getGameRules().getBoolean(TwilightForestMod.ENFORCED_PROGRESSION_RULE)) {
				...
				if (!isSafeBiomeAt(pos, par1Entity)) {
					ModLogger.debug("Portal destination looks unsafe, rerouting!");
					...
					if (safeCoords != null) {
						...
						ModLogger.debug("Safely rerouted!");
					}
					else {
						ModLogger.debug("Did not find a safe spot at first try, trying again with longer range.");
						safeCoords = findSafeCoords(400, pos, par1Entity);
						if (safeCoords != null) {
							par1Entity.setLocationAndAngles(safeCoords.getX(), par1Entity.posY, safeCoords.getZ(), 90.0F, 0.0F);

							ModLogger.debug("Safely rerouted to long range portal.  Return trip not guaranteed.");
						}
						else {
							ModLogger.debug("Did not find a safe spot.");
						}
					}
				}
	...
	private BlockPos findSafeCoords(int range, BlockPos pos, Entity par1Entity) {
		for (int i = 0; i < 25; i++) {
			BlockPos dPos = new BlockPos(pos.getX() + random.nextInt(range) - random.nextInt(range), 100, pos.getZ() + random.nextInt(range) - random.nextInt(range));
			...
			for (int i1 = -i; i1 <= i; ++i1) {
				...
				for (int j1 = -i; j1 <= i; ++j1) {
					for (BlockPos blockpos1 = blockpos3.add(i1, world.getActualHeight() - 1 - blockpos3.getY(), j1); blockpos1.getY() >= 0; blockpos1 = blockpos2) {
						...
						if (world.getBlockState(blockpos1).getBlock() == ModBlocks.NEW_TF_PORTAL) {
							for (blockpos2 = blockpos1.down(); world.getBlockState(blockpos2).getBlock() == ModBlocks.NEW_TF_PORTAL; blockpos2 = blockpos2.down()) {
								blockpos1 = blockpos2;
							}
	...
	private boolean isBlockPortal(World world, BlockPos pos) {
		return world.getBlockState(pos).getBlock() == ModBlocks.NEW_TF_PORTAL;
	}
	...
	public boolean makePortal(Entity entity) {
		...
		if (spot != null) {
			ModLogger.debug("Found ideal portal spot");
			makePortalAt(world, spot);
			return true;
		}
		else {
			ModLogger.debug("Did not find ideal portal spot, shooting for okay one");
			spot = findPortalCoords(entity, false);
			if (spot != null) {
				ModLogger.debug("Found okay portal spot");
				makePortalAt(world, spot);
				return true;
			}
		}
		...
		ModLogger.debug("Did not even find an okay portal spot, just making a random one");
